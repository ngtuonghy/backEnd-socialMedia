...
// CREATE ARTICLE
cloudinary.getUser = async (username) => {
	return await db.query(
		"SELECT * FROM users JOIN user_information ON users.user_id = user_information.user_id WHERE users.username = $1",
		[username],
	);
};
cloudinary.getEmail = async (email) => {
	return await db.query("SELECT * FROM users WHERE email = $1", [email]);
};
cloudinary.getUserbyId = async (userId) => {
	return await db.query(
		"SELECT * FROM users JOIN user_information ON users.user_id = user_information.user_id WHERE users.user_id = $1",
		[userId],
	);
};

cloudinary.update = async (
	username,
	hashedPassword,
	email,
	verifiedEmail,
