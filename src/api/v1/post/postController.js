...
import { generatePublicId } from "#src/utils/utilCreateNanoId";
...
export async function createPost(req, res) {
	const userId = req.cookies.userId;
	const { privacy, text, imageUrl, postId } = req.body;
	try {
		// const postId = generatePublicId(20);
		await post.createPost(postId, userId, privacy, text);

		for (const file of imageUrl) {
			const mediaId = generatePublicId(20);
			await post.createMedia(userId, mediaId, postId, file.mediaType, file.url);
		}
